<?php
  ...
  */session_start();
	...
	if (!isset($_POST['ok']) && !isset($_GET['action']))
	{
		print "<html>";
		print "	<BR><BR>";
		print "	<B><center>::: Relatorio de Avaliacao dos Chamados :::</center></B><BR><BR>";
	...
	if (isset($_POST['ok'])) {
		...
		else
		{
			...
			if($d_ini_completa <= $d_fim_completa)
			{
				...
				$query = "SELECT OCORRENCIA, DATA, LOGIN, COD_AVALIACAO, DESC_AVALIACAO from avaliacao WHERE data >= '".$d_ini_completa."' AND data <= '".$d_fim_completa."'";
...
//				print "</table>";
				...
				print "<TH ".$valign." width='10%'>Data</TH>";
				print "<TH ".$valign." width='10%'>Avaliado por</TH>";
				print "<TH colspan='1' ".$valign." width='10%'>Avaliação</TH>";
				print "<TH colspan='2' width='20%'>Descrição</TH>";
				...
				while ( $row = mysql_fetch_array($sql) ){
					...
					print "<TR>";
					print "<td onclick=\"mostra_ocorrencia($COD);\" align='center' STYLE=\"cursor: pointer;\">";
					print "".$row['OCORRENCIA']."</td>";
					...
					$queryB = mysql_query("SELECT nome FROM usuarios WHERE user_id = '".$row['LOGIN']."'");
					...
					print "</td>";
					$cod_aval = $row['COD_AVALIACAO'];
					print "<TD align='center'>" .$indicadores[$cod_aval]. "</TD>";
//					print "<TD align='center'>".$row['DESC_AVALIACAO']."</TD>";
					$max_char = 50;
					$descricao = $row['DESC_AVALIACAO'];
					$desc = $descricao;
                                        if ( strlen($descricao) > $max_char ) {
                	                        $desc = substr($descricao,0,$max_char);
                        	                $desc .= " (...)";
		                	}
		                	print "<TD colspan='2' width='20%' align='center' onClick=\"mostra_desc('$descricao');\"> $desc </TD>";
					print "</tr>";
	...
	?>
		...
		function mostra_desc(sDescricao){
			var msg = "<div align='center' valign='center'><font color='navy'><p>" + sDescricao + "</p></font></div>";
			$(function(){
				$( msg ).dialog({
					title: 'Descricao da Avaliacao',
					height: 150,
					width: 300,
					modal: true,
					open: function(){
		...
		function mostra_ocorrencia(numero){
			var url = "./mostra_ocorrencia.php?cod=" + numero;
			$(function(){
				$("<p> </p>").dialog({
