	...
	if (isset($_POST['submit']) && $_POST['submit'] == TRANS("BT_SEND")){
		...
		for ($i =1; $i<=$_POST['totalLists']; $i++){
			...
			if (isset($_POST["check_mlist".$i])) {
				...
				if (mail_send($rowconfmail,$rowList['ml_addr_to'],$rowList['ml_addr_cc'],$_POST['field_subject'],$_POST['field_body'], $rowMailArea['replyto'])) {
					...
					$sqlHist = "INSERT INTO mail_hist (mhist_oco, mhist_listname, mhist_address, mhist_address_cc, mhist_subject, mhist_body, mhist_date, mhist_technician) ".
								" values ('".$_POST['numero']."', '".$rowList['ml_sigla']."', '".$rowList['ml_addr_to']."', '".$rowList['ml_addr_cc']."', '".noHtml($_POST['field_subject'])."', '".noHtml($_POST['field_body'])."', '".date("Y-m-d H:i:s")."', '".$_SESSION['s_uid']."')";
		...
		if (isset($_POST['field_to_editable']) && $_POST['field_to_editable']!=""){
			...
			if (mail_send($rowconfmail,$_POST['field_to_editable'],'',$_POST['field_subject'],$_POST['field_body'], $rowMailArea['replyto'])) {

				$sqlHist = "INSERT INTO mail_hist (mhist_oco, mhist_listname, mhist_address, mhist_address_cc, mhist_subject, mhist_body, mhist_date, mhist_technician) ".
							" values ('".$_POST['numero']."', '', '".noHtml($_POST['field_to_editable'])."', '', '".noHtml($_POST['field_subject'])."', '".noHtml($_POST['field_body'])."', '".date("Y-m-d H:i:s")."', '".$_SESSION['s_uid']."')";
