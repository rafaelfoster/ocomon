<? /*                        Copyright 2005 Flávio Ribeiro
  ...
  */session_start();
	...
	if ($_SESSION['s_allow_change_theme'] == 0){
		print "<script>mensagem('Sinto muito!\\nA alteração de temas foi desabilitada pelo administrador do sistema!'); history.back();</script>";
		exit;
	}
	...
	$execUserTheme = mysql_query($sqlUserTheme) or die ('ERRO NA TENTATIVA DE RECUPERAR AS INFORMAÇÕES DA TABELA DE TEMAS DO USUÁRIO!');
	$rowUth = mysql_fetch_array($execUserTheme);
	$hasTheme = mysql_num_rows($execUserTheme);


	print "<BR><B>TEMAS: &nbsp;</b><BR>";
	...
	if (!isset($_POST['submit'])) {
		...
		$exec = mysql_query($sql) or die('ERRO NA TENTATIVA DE RECUPERAR AS INFORMAÇÕES DE TEMAS!');
		$qtd = mysql_num_rows($exec);
		if ($qtd==0){
			print "<script>mensagem('Não há temas salvos no sistema!'); window.self.close();</script>";
			exit;
		}

		print "<tr><td>TEMA:</td><td>";
		...
		print "<td align='center'><input type='submit' name='submit' class='button' value='Carregar'></td>".
				"<td align='center'><input type='button' name='cancelar' class='button' value='Cancelar' onClick=\"javascript:history.back();\"></td>";
		print "</tr>";
...
/*		if ($_POST['tema'] == -1){
		...
		if ($_POST['tema'] == -1){
			...
			$execU = mysql_query($sqlU) or die ('NÃO FOI POSSÍVEL ACESSAR A TABELA DE TEMAS!<BR>'.$sqlU);
			print "<script>mensagem('Tema alterado com sucesso!'); window.open('../../index.php','_parent','');  </script>";
			exit;
		} //else
		//if ($_POST['tema'] == -3){
		...
		if ($hasTheme>0){
			...
			$exec = mysql_query($sql) or die('ERRO NA TENTATIVA DE ATUALIZAR O DADOS DO REGISTRO!<br>'.$sql);
		} else {
		...
		$execQry = mysql_query($qry) or die ('ERRO NA TENTATIVA DE RECUPERAR AS INFORMAÇÕES DO TEMA!<BR>'.$qry);
		...
		print "<script>mensagem('Tema carregado com sucesso!'); window.open('../../index.php','_parent','');  </script>"; //?LOAD=ADMIN
...
<!--
	function valida(){
		...
		var ok = validaForm('idNomeTema','COMBO','Nome do Tema',1);
