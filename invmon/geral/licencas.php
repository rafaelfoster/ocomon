<?php 
  ...
  */session_start();
	...
	print "<BR><B>Administração de tipos de Licenças de Softwares</B><BR>";
	...
	if ((!isset($_GET['action'])) && empty($_POST['submit'])) {
		...
		print "<TR><TD bgcolor='".BODY_COLOR."'><a href='".$_SERVER['PHP_SELF']."?action=incluir&cellStyle=true'>Cadastrar tipo de Licença</a></TD></TR>";
		...
		else
		{
			...
			print "Existe(m) <b>".$registros."</b> tipos de licenças cadastradas.</td>";
			print "</tr>";
			print "<TR class='header'><td class='line'>Licença</TD>".
	...
	if ((isset($_GET['action'])  && ($_GET['action'] == "incluir") )&& empty($_POST['submit'])) {
		...
		print "<BR><B>Cadastro de tipos de Licenças</B><BR>";
		print "<TR><TD bgcolor='".BODY_COLOR."'><a href='".$_SERVER['PHP_SELF']."'>Listar tipos de Licença cadastrados</a></TD></TR>";
		print "<TR>";
	...
	if ((isset($_GET['action']) && $_GET['action']=="alter") && empty($_POST['submit'])) {
		...
		print "<TR>";
                print "<TD width='20%' align='left' bgcolor='".TD_COLOR."' valign='top'>Licença:</TD>";
...
<!--
	function valida(){
		var ok = validaForm('idLicenca','','Descrição',1);
